...
function cuSel(params) {
	...
	*/
    ...
    if(chElParent.get(0).tagName.toLowerCase()=='form') {  // также удаляем форму, если она - родитель элемента
        chEl = chEl.parent();
    }
...
jQuery("html").click(
	function(e)
	{
        ...
        /*
         изменение "Вакансии по областям науки"
         */
		else if(clickedClass=='item-category__title-elem collapsed')
        {
            $(clicked).parents('.panel-default').toggleClass('selected');
        ...
        /*
         изменение "Вакансии по областям науки"
         */
        else if(clickedClass=='item-category__title-elem')
        {
            $(clicked).parents('.panel-default').toggleClass('selected');
            $(clicked).parents('.panel-default').find('.panel-collapse').toggleClass('collapse').css('height','auto');
