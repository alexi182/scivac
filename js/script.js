 ...
 */
    ...
    $('.jshelper-sub-research-directions').click(function () {
        ...
        var parentContainer2 = $(source).parents('.panel-collapse');
        $(parentContainer).siblings('.jshelper-list-sections-science').hide();
        $(parentContainer2).css('height','auto');
        $(parentContainer).siblings('.jshelper-list-sections-science[id="' + $(source).attr('data-tabname') + '"]').show();
    });

    $('.cuselScrollArrows').change(function () {
        if(!$(this).parent().parent().parent().find('.migan')[0]) {
            var date2 = parseInt($(this).parent().parent().parent().find('.cuselScrollArrows:eq(1)').find('.cuselText').html());
            var date1 = parseInt($(this).parent().parent().parent().find('.cuselScrollArrows:eq(0)').find('.cuselText').html());
            if (date2 < date1) {
                $(this).parent().parent().parent().find('.cuselScrollArrows:eq(1)').find('.cuselText').html('');
                if ($(this).parent().parent().parent().find('.cuselScrollArrows:eq(1)').find('.error').data('error') != 'yes') {
                    $(this).parent().parent().parent().find('.cuselScrollArrows:eq(1)').prepend('<div class="error" data-error="yes">Год окончания не может быть меньше года начала</div>');
                } else {
                    $(this).parent().parent().parent().find('.cuselScrollArrows:eq(1)').find('.error').show();
                }
            } else {
                $(this).parent().parent().parent().find('.cuselScrollArrows:eq(1)').find('.error').hide();
            }
        }
    });
    ...
    $('#fos_user_registration_form_username').change(function () {
        var elem = $(this);
        $.post('/public/vacancies/check/', {mode: 'username',name: elem.val()}, function (v) {
            elem.parent().parent().find('.field-validation-error').html(v);
        }, 'json');
    });

    $('#fos_user_registration_form_email').change(function () {
        var elem = $(this);
        $.post('/public/vacancies/check/', {mode: 'mail',name: elem.val()}, function (v) {
            elem.parent().parent().find('.field-validation-error').html(v);
        }, 'json');
    });
    ...
    function toggleChevron(e) {
        $(e.target).prev('.panel-heading').parent().find('.panel-collapse').toggleClass('collapse');
    }
...
function getResearchDirectionsStatistics(childId) {
    $("[aria-labelledby='heading_"+childId+"']").find('div.item-direction[data-researchdirectionid]').each(function () {
        ...
        $.ajax({
            url: "/public/vacancies/analytics/" + id,
            success: function (data) {
                var parentDiv = $('div.item-direction[data-researchdirectionid="' + id + '"]');
                $(parentDiv).find("span.totalvacancies").html(data.cnt);
                if(data.pay>0) { $(parentDiv).find("span.averagesalaries").html(data.pay); } else { $(parentDiv).find("span.averagesalaries").html(0); }
            }
        });
...
function refreshAllGraphicData() {
    ...
    if (graphs.chart !== null) {
        ...
        graphs.chart.render();
        $.get("/public/vacancies/api/", dataGraph1, function (data) {
    ...
    if (graphs.chart2 != null) {
        ...
        $.get("/public/vacancies/api/", dataGraph2, function (data) {
...
function addingNewItemToList(parentDiv, prefix) {
    ...
    var newPrefixDash = prefix + "_" + newIndex + "__",
    ...
    prefix = prefix.replace('\\[','');
    prefix = prefix.replace('\\]','');
    var newPrefixSelect = prefix + "_" + newIndex + "__",
        oldPrefixSelect = prefix + "_0__";
    ...
    var templateDivInnerHtml = templateDiv
            ...
            .replace(new RegExp(oldPrefixSelect, 'g'), newPrefixSelect)
